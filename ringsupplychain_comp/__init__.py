...
class Player(BasePlayer):
    ...
    def get_predecessor(self, predecessor_num=1):
        p_per_group = self.subsession.players_per_group
        if predecessor_num == 1:
            return self._get_r1(self.id_in_group, p_per_group)
        else:
            half = p_per_group // 2
            if self.id_in_group > half:
                return self._get_r1(self.id_in_group - half, p_per_group)
            else:
                return self._get_r1(self.id_in_group + half, p_per_group)
            
    def get_successor(self, successor_num=1):
        p_per_group = self.subsession.players_per_group
        if successor_num == 1:
            return self._get_s1(self.id_in_group, p_per_group)
        else:
            half = p_per_group // 2
            if self.id_in_group > half:
                return self._get_s1(self.id_in_group - half, p_per_group)
            else:
                return self._get_s1(self.id_in_group + half, p_per_group)
...
def live_request(player, data):
    take_from = player.get_predecessor(predecessor_num=data['predecessor'])
    # print("player_id", player.id_in_group, "take_from_player_id:", take_from, "predecessor_num:", data['predecessor'])
    units = data['units']
...
def common_vars_for_template(player):
    ...
    return {
        ...
        'DEBUG': DEBUG,
        'predecessor1': player.get_predecessor(predecessor_num=1),
        'predecessor2': player.get_predecessor(predecessor_num=2),
        'successor1': player.get_successor(successor_num=1),
        'successor2': player.get_successor(successor_num=2),
    }
